...
- **排行榜**：按已解题目积分自动排名（简单 1 分 / 中等 2 分 / 困难 3 分），展示积分、已解数与尝试次数
...
题库共 **100 题**（简单 39 · 中等 39 · 困难 22），难度全局递进：
**第 1-39 题（简单）→ 第 40-78 题（中等）→ 第 79-100 题（困难）**；
每题含 3-5 组可见示例用例与 7-10 组隐藏用例（隐藏用例仅返回对错，不下发内容）。
...
- **答案保护**：每题 10-15 组测试用例，前端仅下发 3-5 组示例用例；隐藏用例不下发，判题后只返回对错，F12 无法偷看预期输出
